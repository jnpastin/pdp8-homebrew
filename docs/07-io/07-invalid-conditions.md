...
- `DMA_ENABLE` asserted outside the timing contexts defined by the DMA arbiter contract
- aggregate `/DMA_REQ` generated by the arbiter rather than the separate combinational aggregation logic
- aggregate `/DMA_REQ` not continuously derived from `DMA_ENABLE` and `/DMA_REQ[14:0]`
